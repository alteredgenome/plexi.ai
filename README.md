...
Install Plexi and all dependencies with a single command:

```bash
curl -fsSL https://alteredgenome.github.io/plexi.ai/install.sh | bash
```
*(Or via `curl -fsSL https://plexi.fyi/install.sh | bash`)*

Once installed, open `http://localhost:8000` in your browser. Plexi's interactive **First-Run Setup Wizard** will guide you through setting up your administrator credentials, OpenRouter AI keys, and smart home connectors.
...
- ⚡ **Pavlok 3 Haptic Momentum:** Triggers haptic vibration, beeps, or gentle reminders when critical momentum tasks are overdue.
...
```bash
git clone https://github.com/alteredgenome/plexi.ai.git
cd plexi.ai
